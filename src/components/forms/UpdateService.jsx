...
import {TextField, Button, Select, MenuItem, Checkbox, Switch} from '@material-ui/core'
...
export default function UpdateService({closeForm, formProps}) 
{
    ...
    const [category, setCategory] = useState(initial.categoryName === '' ? 'Uden Kategori' : initial.categoryName)
    const [elgibleCalendars, setElgibleCalendars] = useState(initial.elgibleCalendars)
    ...
    useEffect(() =>
    {
        getAllCalendars(localStorage.getItem('apiKey'), abortController).then((res) => setCalendars(res.map((calendar) => 
        {
            return {
                id: calendar._id,
                name: calendar.name
    ...
    const handleSubmit = () =>
    {
        ...
        updateService(localStorage.getItem('apiKey'), data, initial._id).then(() => closeForm()).catch((err) => setError(err.response.data.msg))
    }
        ...
        >
            <div className="flex flex-col justify-center items-center w-full px-6 pb-4">
                ...
                <div className="flex flex-col md:flex-row w-full justify-between items-center">
                    ...
                    <span className="md:h-64 mx-4 my-2 flex bg-blue-700 md:w-border opacity-50">
                    ...
                    <div className="flex w-full md:w-1/2 flex-col justify-between items-center">
                        ...
                        >
                            {categoryList.map((category, i) => <MenuItem key={i} value={category.name}>{category.name}</MenuItem>)}
                        </Select>
                        ...
                        {calendars ? 
                            <Autocomplete
                                ...
                                renderInput={(params) => {
                                    return <TextField {...params} variant="outlined" label="Hvem kan udføre servicen" />
                                }}
                            />
                        : null}
                        ...
                        <i className="MuiFormHelperText-root not-italic">Giver kunder mulighed for at booke servicen online, ellers kan den kun bookes internt</i>
                    </div>
                </div>
